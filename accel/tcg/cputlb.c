...
void tlb_flush_counts(size_t *pfull, size_t *ppart, size_t *pelide)
{
    ...
    CPU_FOREACH(cpu) {
        ...
        full += atomic_read(&env->tlb_full_flush_count);
        part += atomic_read(&env->tlb_part_flush_count);
        elide += atomic_read(&env->tlb_elide_flush_count);
    }
    *pfull = full;
    *ppart = part;
    *pelide = elide;
...
static void tlb_flush_by_mmuidx_async_work(CPUState *cpu, run_on_cpu_data data)
{
    ...
    tlb_debug("mmu_bitmask: 0x%" PRIx16 "\n", asked);

    qemu_spin_lock(&env->tlb_lock);

    /* Reduce the set of tlbs to flush by those that are known clean.  */
    all_dirty = env->tlb_dirty;
    to_clean = asked & all_dirty;
    all_dirty &= ~to_clean;
    env->tlb_dirty = all_dirty;
    env->tlb_pending_flush &= ~all_dirty;
    ...
    if (to_clean == ALL_MMUIDX_BITS) {
        atomic_set(&env->tlb_full_flush_count, env->tlb_full_flush_count + 1);
    } else {
        atomic_set(&env->tlb_part_flush_count,
                   env->tlb_part_flush_count + ctpop16(to_clean));
    }
    if (to_clean != asked) {
        atomic_set(&env->tlb_elide_flush_count,
                   env->tlb_elide_flush_count + ctpop16(asked & ~to_clean));
    }
...
void tlb_flush_by_mmuidx(CPUState *cpu, uint16_t idxmap)
{
    ...
    if (cpu->created && !qemu_cpu_is_self(cpu)) {
        ...
        qemu_spin_lock(&env->tlb_lock);
        pending = env->tlb_pending_flush;
        to_clean = idxmap & ~pending;
        env->tlb_pending_flush = pending | idxmap;
        if (to_clean != idxmap) {
            atomic_set(&env->tlb_elide_flush_count,
                       env->tlb_elide_flush_count +
                       ctpop16(idxmap & ~to_clean));
        }
        qemu_spin_unlock(&env->tlb_lock);

        if (to_clean) {
            tlb_debug("reduced mmu_bitmask: 0x%" PRIx16 "\n", to_clean);
...
static void tlb_flush_page_locked(CPUArchState *env, int midx,
                                  target_ulong addr)
{
    target_ulong lp_addr = env->tlb_desc[midx].large_page_addr;
    target_ulong lp_mask = env->tlb_desc[midx].large_page_mask;
...
{
    target_ulong lp_addr = env->tlb_desc[mmu_idx].large_page_addr;
    ...
    if (lp_addr == (target_ulong)-1) {
        ...
        lp_mask &= env->tlb_desc[mmu_idx].large_page_mask;
        while (((lp_addr ^ vaddr) & lp_mask) != 0) {
            lp_mask <<= 1;
        }
    }
    env->tlb_desc[mmu_idx].large_page_addr = lp_addr & lp_mask;
    env->tlb_desc[mmu_idx].large_page_mask = lp_mask;
}
