                             ...
                             bool is_read)
{
    ...
    tcg_debug_assert(mask_ofs <= 0xffffff);
    tcg_debug_assert(table_ofs <= 0xffffff);
    mask_hi = mask_ofs & 0xfff000;
    table_hi = table_ofs & 0xfff000;

    if (mask_hi || table_hi) {
        table_base = TCG_REG_X1;
        mask_base = (mask_hi == table_hi ? table_base : TCG_REG_X0);
