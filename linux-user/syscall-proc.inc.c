...
SYSCALL_IMPL(getgroups)
{
    ...
    if (gidsetsize == 0) {
        return get_errno(getgroups(0, NULL));
    }
    /*
     * If we are passed a comical value like INT_MAX, we do not wish to
     * fail to allocate our temporary memory.  The guest should not have
     * been able to set more than NGROUPS_MAX previously, therefore only
     * reading that amount should not result in EINVAL.
     * NGROUPS_MAX is capped at 64k for Linux 2.6.4 and later.
     */
    if (gidsetsize > NGROUPS_MAX) {
        gidsetsize = NGROUPS_MAX;
    }

    grouplist = g_new(gid_t, gidsetsize);
...
SYSCALL_IMPL(getgroups32)
{
    ...
    /* See above re comically large values of gidsetsize.  */
    if (gidsetsize > NGROUPS_MAX) {
        gidsetsize = NGROUPS_MAX;
    }
